...
TV = 18    # GPIO-18 TV power interrupt to shutdown procedure

# use a weak pull_up to create a high
GPIO.setup(TV, GPIO.IN, pull_up_down=GPIO.PUD_UP)

def main():

    while True:
        # set an interrupt on a falling edge and wait for it to happen
        GPIO.wait_for_edge(TV, GPIO.FALLING)
	time.sleep(60)
	if GPIO.input(TV):
    		print('Input was HIGH, TV still ON')
       	else:
    		print('Input was LOW, TV is effectively OFF')
